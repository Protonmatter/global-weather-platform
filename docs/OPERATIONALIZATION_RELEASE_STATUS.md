...
- Bounded audit-history reads use the same chronological index, so an operator request such as `limit=1` does not replay and sort the lifetime ledger.
- Source-ingestion receipts are the final records in the same fsynced append payload as their canonical observation batch. Interrupted writes can expose observations without a receipt for conservative recovery, but can never expose a receipt before its observations.
